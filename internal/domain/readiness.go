...
func task20CancelState(state ExecutionRequestState) bool {
	// BUG: executing requests are admitted to the cancellation path.
	return state == ExecutionRequestSubmitted || state == ExecutionRequestAuthorized || state == ExecutionRequestExecuting
}
